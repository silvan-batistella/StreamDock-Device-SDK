...
Serviço systemd que inicializa o daemon `ydotoold` (simulação de teclado via uinput)
e em seguida executa o script Python `k1_pro.py`, que controla o dispositivo
StreamDock K1 PRO no Linux.

### Arquivos

| Arquivo                | Descrição                                                  |
|------------------------|------------------------------------------------------------|
| `start_streamdock.sh`  | Wrapper que inicia o `ydotoold` e depois o `k1_pro.py`     |
...
sudo apt install -y python3 libudev-dev libusb-1.0-0-dev libhidapi-libusb0

# ydotool (simulação de teclado compatível com Wayland e X11)
...
O StreamDock K1 PRO precisa de acesso ao `/dev/hidraw*` e o `ydotool` precisa
de acesso ao `/dev/uinput`. Crie as regras udev:
...
```bash
# Adiciona o usuário ao grupo input (necessário para ydotool/uinput)
...
### `ydotool` não funciona (teclas não são enviadas)

```bash
# Verifique se o socket do ydotoold existe
ls -la /tmp/.ydotool_socket /run/user/$(id -u)/.ydotool_socket 2>/dev/null

# Teste manualmente (envia tecla 'a')
...
O README cobre a estrutura do projeto conforme está organizada: [10-cite-0](#10-cite-0) 

As regras udev para o K1 PRO usam o vendor ID `6603` e product IDs `1015`/`1019`, conforme definido em: [10-cite-1](#10-cite-1) [10-cite-2](#10-cite-2) 

As dependências de sistema estão documentadas no README do Python SDK: [10-cite-3](#10-cite-3)